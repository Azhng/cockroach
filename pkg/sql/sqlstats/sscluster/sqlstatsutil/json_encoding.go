...
// BuildStmtMetadataJSON returns a string that constructs a JSONB using
// jsonb_build_object() SQL builtin for the metadata section of the
...
// roachpb.CollectedStatementStatistics into a valid SQL string using
// jsonb_build_object() builtin. The resulting string  can be embedded into a
// SQL statement to create the JSONB object.
//
...
// roachpb.CollectedTransactionStatistics into a valid SQL string using
// jsonb_build_object() builtin. The resulting string  can be embedded into a
// SQL statement to create the JSONB object.
//
// JSON Schema:
...
func buildStmtFingerprintIDArray(stmtFingerprintIDs []roachpb.StmtFingerprintID) json.JSON {
	...
	// TODO(azhng): wip: ensure we will have the same encoding as the PK in
	//  stmt stats table.
	for _, stmtFingerprintID := range stmtFingerprintIDs {
		builder.Add(json.FromString(encodeStmtFingerprintID(stmtFingerprintID)))
	}

	return builder.Build()
}

func encodeStmtFingerprintID(id roachpb.StmtFingerprintID) string {
	result := make([]byte, 0, 8)
	result = encoding.EncodeUint64Ascending(result, uint64(id))
